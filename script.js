(function () {
  ...
  function updateDangoDisplay() {
    const visibleIndexes = [
      ...Array.from(
        { length: Math.max(0, dangoCount - 3) },
        (_, index) => index,
      ),
      ...Array.from(
        { length: Math.min(3, dangoCount) },
        (_, index) => index + 3,
      ),
    ];
    ...
    dangoRows.forEach((row) => {
      row.style.display = [...row.children].some(
        (dango) => dango.style.display !== "none",
      )
        ? "flex"
        : "none";
    });
  ...
  function randomizeRocks() {
    ...
    rockElements.forEach((rock, index) => {
      ...
      for (let attempt = 0; attempt < 30; attempt++) {
        ...
        const overlaps = placedRocks.some(
          (placedLeft) =>
  ...
  function drawMoon() {
    ...
    const mg = sctx.createRadialGradient(
      mx - r * 0.15,
      my - r * 0.15,
      0,
      mx,
  ...
  function makeLeaf(init, fromTree, sourceIndex) {
    ...
    sourceIndex =
      fromTree && sourceIndex !== undefined
        ? sourceIndex % crowns.length
        : fromTree
          ? Math.floor(Math.random() * crowns.length)
          : 0;
    const crown = crowns[sourceIndex];
    return {
      x: fromTree
        ? crown.left + Math.random() * crown.width
        : Math.random() * W,
      y: init
        ? fromTree
          ? crown.top + Math.random() * crown.height
          : Math.random() * H
        : fromTree
          ? crown.top + Math.random() * crown.height
          : -20,
      ...
      osc: Math.random() * 80,
      color:
        TREE_LEAF_COLORS[Math.floor(Math.random() * TREE_LEAF_COLORS.length)],
      floorY: H + 10,
      fromTree,
      sourceIndex,
    };
  ...
  function drawLeaves() {
    leaves.forEach((l, i) => {
      ...
      if (l.y > l.floorY)
        leaves[i] = makeLeaf(false, l.fromTree, l.sourceIndex);
  ...
  function drawSusuki() {
    ...
    susuki.forEach((s) => {
      ...
      for (let j = 0; j < 11; j++) {
        const t = j / 11;
        const px =
          (1 - t) * (1 - t) * s.x + 2 * (1 - t) * t * cX + t * t * tipX;
        const py =
          (1 - t) * (1 - t) * s.baseY + 2 * (1 - t) * t * cY + t * t * tipY;
